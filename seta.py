...
__all__ = ['InfoBatch', 'SeTa', 'BLS', 'prune']
...
def prune(dataset, args):
    ...
    elif 'BLS' in args.prune_type:
        if 'InfoBatch' in args.prune_type or 'mean' in args.prune_type:
            instance = InfoBatch(dataset, args.epochs, args.prune_ratio, args.delta)
            print(f'==> BLS_InfoBatch: ratio={args.prune_ratio}, delta={args.delta}')
        elif 'SeTa' in args.prune_type:
            instance = SeTa(dataset, args.epochs, args.prune_ratio,
                            args.num_group, args.window_scale,
                            args.delta)
            print(f'==> BLS_SeTa: ratio={args.prune_ratio}, group={args.num_group}, '
                  f'window_scale={args.window_scale}, delta={args.delta}')
        else:
            raise ValueError(f'Unknown InfoBatch type: {args.prune_type}')
        dataset = BLS(instance, args.alpha).proxy()
        print(f'==> BLS: alpha={args.alpha}')
        
        assert 0 <= args.alpha < 1, 'For BLS, alpha (ema) must be in [0, 1)'
...
class BLS:
    def __init__(
        ...
        """
        BLS is a wrapper of InfoBatch.
        It is used to update the loss of the samples.
        :param instance: InfoBatch instance
        :param alpha: float, the alpha value
        """
        self.alpha = alpha
        self.instance = instance
        self.instance.scores[:] = 0

    def update(self, values):
        assert isinstance(values, torch.Tensor)
        instance = self.instance
        
        # if values is batch loss, we need to calculate the mean loss
        if values.dim() == 1:
            values = values.mean()
        device = values.device
        weights = instance.weights[instance.cur_batch_index].to(device)
        indices = instance.cur_batch_index.to(device)
        instance.cur_batch_index = []
        loss_val = values.detach().clone()

        if dist.is_available() and dist.is_initialized():
            if loss_val.dim() == 0:
                loss_val = loss_val.expand(len(indices))
            iv = torch.cat([indices.view(1, -1), loss_val.view(1, -1)], dim=0)
            iv_whole_group = concat_all_gather(iv, 1)
            indices = iv_whole_group[0]
            loss_val = iv_whole_group[1]
        ...
        instance.scores[indices] = self.alpha * instance.scores[indices] \
            + (1 - self.alpha) * loss_val.cpu()
        
        values.mul_(weights.mean())
        return values
    
    def proxy(self):
        self.instance.update = self.update # override
