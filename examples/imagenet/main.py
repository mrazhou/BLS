...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['IMNET', 'INAT', 'INAT19'],
    ...
    # for seta
    parser.add_argument('--prune_type', type=str, default='SeTa')
    parser.add_argument('--prune_ratio', type=float, default=0.0)
    parser.add_argument('--delta', type=float, default=0.875)
    parser.add_argument('--num_group', type=int, default=10)
    parser.add_argument('--window_scale', type=float, default=0.6)
...
def main(args):
    ...
    if args.distributed:
        ...
        if args.repeated_aug:
            ...
            print('Cannot use repeated_aug with SeTa. This setting will be ignored!!')
            # exit(-1)
        else:
    ...
    dataset_train = prune(dataset_train, args)
    data_loader_train = torch.utils.data.DataLoader(
        dataset_train, sampler=dataset_train.sampler,
    ...
    data_loader_val = torch.utils.data.DataLoader(
        ...
        batch_size=int(1.5 * args.batch_size * 1),
        num_workers=args.num_workers,
        pin_memory=args.pin_mem,
        drop_last=False
    )
    ...
    model = create_model(
        ...
        img_size=args.input_size if 'resnet' not in args.model else None
    )
...
if __name__ == '__main__':
    ...
    with tlog(args.output_dir) as tslog:
        main(args)
